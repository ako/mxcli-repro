...
```bash
./mxcli -p Repro1024.mpr    # Correct - uses local binary
```
...
```bash
./mxcli setup mxbuild -p Repro1024.mpr    # Auto-download for project's Mendix version
```

After setup, `mx` is at `~/.mxcli/mxbuild/{version}/modeler/mx`. Usage:

```bash
~/.mxcli/mxbuild/*/modeler/mx check Repro1024.mpr   # Validate project
./mxcli docker check -p Repro1024.mpr               # Alternative (auto-downloads mxbuild)
```
...
```bash
./mxcli -p Repro1024.mpr -c "SHOW MODULES"              # List all modules
./mxcli -p Repro1024.mpr -c "SHOW STRUCTURE"             # Project overview
./mxcli -p Repro1024.mpr -c "SHOW ENTITIES IN MyModule"  # Entities in a module
./mxcli -p Repro1024.mpr -c "DESCRIBE ENTITY MyModule.Customer"  # Entity details
```

### Execute an MDL Script File

```bash
./mxcli exec script.mdl -p Repro1024.mpr
```
...
```bash
./mxcli
# Then: CONNECT LOCAL 'Repro1024.mpr';
```
...
./mxcli check script.mdl -p Repro1024.mpr --references  # With reference validation
```
...
```bash
./mxcli check script.mdl -p Repro1024.mpr --references
```
...
```bash
# Lint the project
./mxcli lint -p Repro1024.mpr

# With colored output
./mxcli lint -p Repro1024.mpr --color

# List available rules
./mxcli lint -p Repro1024.mpr --list-rules

# Output as SARIF
./mxcli lint -p Repro1024.mpr --format sarif > results.sarif
```
...
./mxcli report -p Repro1024.mpr

# JSON report
./mxcli report -p Repro1024.mpr --format json

# HTML report
./mxcli report -p Repro1024.mpr --format html
```
