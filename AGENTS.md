...
```bash
./mxcli -p ReproApp.mpr    # Correct - uses local binary
```
...
```bash
./mxcli setup mxbuild -p ReproApp.mpr    # Auto-download for project's Mendix version
```

After setup, `mx` is at `~/.mxcli/mxbuild/{version}/modeler/mx`. Usage:

```bash
~/.mxcli/mxbuild/*/modeler/mx check ReproApp.mpr   # Validate project
./mxcli docker check -p ReproApp.mpr               # Alternative (auto-downloads mxbuild)
```
...
```bash
./mxcli -p ReproApp.mpr -c "SHOW MODULES"              # List all modules
./mxcli -p ReproApp.mpr -c "SHOW STRUCTURE"             # Project overview
./mxcli -p ReproApp.mpr -c "SHOW ENTITIES IN MyModule"  # Entities in a module
./mxcli -p ReproApp.mpr -c "DESCRIBE ENTITY MyModule.Customer"  # Entity details
```

### Execute an MDL Script File

```bash
./mxcli exec script.mdl -p ReproApp.mpr
```
...
```bash
./mxcli
# Then: CONNECT LOCAL 'ReproApp.mpr';
```
...
./mxcli check script.mdl -p ReproApp.mpr --references  # With reference validation
```
...
```bash
./mxcli check script.mdl -p ReproApp.mpr --references
```
...
```bash
# Lint the project
./mxcli lint -p ReproApp.mpr

# With colored output
./mxcli lint -p ReproApp.mpr --color

# List available rules
./mxcli lint -p ReproApp.mpr --list-rules

# Output as SARIF
./mxcli lint -p ReproApp.mpr --format sarif > results.sarif
```
...
./mxcli report -p ReproApp.mpr

# JSON report
./mxcli report -p ReproApp.mpr --format json

# HTML report
./mxcli report -p ReproApp.mpr --format html
```
